...
description: documenter equivalent running on Ollama (minimax-m3:cloud). Writes README/API docs in the project's existing style. Cheap fast writing. Examples — "have ollama-documenter write a README", "use ollama-documenter to document the API".
model: haiku
---
...
3. Run via Bash heredoc:
   ...
   <user task>
   PROMPT_EOF
   ollama launch claude --model minimax-m3:cloud -- -p "$PROMPT" --permission-mode acceptEdits
   ```
