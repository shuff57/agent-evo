...
**A stored voice profile ships with this skill: `voice-shuff.md`, in this
directory.** It is Steven Huff's voice, built from his own writing, and it is
included as a worked example of what a profile looks like and as a usable
plain-professional register for anyone who wants one.

Which profile to use, in order:

1. **A sample the user supplies in the conversation always wins.** Analyse it
   and match it. Never override someone's own writing with a stored profile.
2. **A `voice-*.md` beside this file that the user says is theirs.** Read it and
   treat it as the sample.
3. **`voice-shuff.md`**, only when the user has asked for it by name, or has
   asked for a plain professional register with no sample of their own.

The point of a profile is that it records which of the patterns below are
genuine habits of a particular writer and must be PRESERVED rather than
stripped. That makes a profile useless — worse than none — applied to someone
it was not built from. Do not reach for `voice-shuff.md` by default.
