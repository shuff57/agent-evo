...
**Only ONE of those guards is worth porting to Claude Code, and it is the loop guard.** Probing `claude.exe` (2026-09-16) found the harness already ships the other four natively, sometimes with better wording than a port would have: `BASH_MAX_OUTPUT_LENGTH` / `TASK_MAX_OUTPUT_LENGTH` / `MAX_MCP_OUTPUT_TOKENS` and `[OUTPUT TRUNCATED - exceeded N token limit]` notices; an edit failure that says "String to replace not found in file" **and** tells the model to re-read and copy the exact surrounding text; "agent stalled on all N attempts (no progress for Xms each)" and "agent abandoned"; and "was called with input that could not be parsed as JSON … Retry with valid JSON". Repeated-identical-call detection is the one thing absent — the obvious strings are nowhere in the binary, and the single "Do not retry the identical call" is Chrome-extension-specific. So `hooks/loop-guard.js` is the whole Claude-side port; the other four exist only on the opencode side. Do not add Claude-side equivalents without re-probing first: duplicating a native behavior costs a subprocess on every tool call and teaches the model nothing.
...
**Claude Code now has the equivalent hook, and it is wired.** `hooks/peer-inbox.js` is registered
as a matcher-less `PostToolUse` hook in `settings.json`, so a cross-agent message reaches a long
Claude turn the same way it reaches an opencode run. The cost objection that parked it is answered
by an mtime gate: the common case is "nothing new", and it ends after one `stat` of the box log
with no subprocess spawned. Only when the log's mtime moved does it spawn `msg.mjs inbox --as
claude`, which owns the cursor, the unread filter and the formatting — the hook never reimplements
them, so the two can never disagree about what counts as unread. Delivery rides `additionalContext`
on the tool result, the same channel tier-gate, context-monitor and loop-guard use. It fails open
everywhere: no box, no log, no `msg.mjs`, a malformed payload, a non-zero child or an unwritable
state file all exit 0 silently, because a mailbox that breaks a tool call is worse than a missed
message.

## Peer bridge

The message center is the durable, cross-machine channel; the peer bridge is the live, same-box
one. It speaks **peerProtocol v1**: an auth frame followed by a user frame, one JSON object per
line (NDJSON), over a Unix-domain socket. The codec (`bin/peer/codec.mjs`) and registry
(`bin/peer/registry.mjs`) are the transport units; the sidecar and CLI are the two ends.

**Registry and keys live under `.msgbox/peer/`** (`registry.json` plus one `<peerId>.key` per
peer), beside `log.jsonl`. That directory is device-local and gitignored — `.gitignore` keeps
`.msgbox/*` and re-includes only `log.jsonl` — so keys never ship between machines.

**Sidecar** — one per lane per box, relays inbound frames into the box log:

```
node bin/peer-sidecar.mjs --as opencode [--heartbeat-ms 30000]
```

**Peer CLI** — outbound send, list, sweep, unregister:

```
node bin/peer.mjs send --to <name|pid> --text <s> [--priority now|next|later] [--from-name <s>] [--no-audit]
```

**Identity is fail-closed.** The auth frame carries the sender's own key-file contents; the
receiver hashes it and matches it against a managed registry entry's `keyHash`. A match names the
connection; anything else — no match, no registry, unreadable key — is destroyed silently and
nothing is appended. The log's `from` is always the registry-verified name; the envelope's
`from`/`from-name`/`from-mode` attributes are display-only and never authorize. The trust boundary
is the same OS user and nothing more: 0600 key files, a 0700 socket directory, and no cross-user
guarantee.

**Priority is honest.** The wire carries `now|next|later` for protocol compatibility, and the
value is recorded on the entry — but there is no interrupt channel, so delivery is the next tool
call for all three. `now` does not preempt a running turn.

**Run the peer suites with `bun test`, not `node --test`.** On a box where `node` is a shim to
bun — this one — `node --test <file>` runs the file with no test runner at all and `node:test`
throws on the first case, which reads as a broken suite rather than a wrong command. `bun test`
works either way, so it is what every peer test header names.

### Watchable lane (tmux)

The sidecar is a socket: fast and invisible. `bin/peer-term.mjs` is the same conversation in a
window you can attach to — one tmux session per lane, the prompt typed in, the reply rendered
live, the text returned on stdout:
...
`tmux attach -t peer-<lane>` shows the exchange at any point. Three properties are worth knowing
before changing it:
...
- **Completion is a fact, not a heuristic.** The pane runs a generated script that records
  ...
  idle-detection, no "output stopped changing" guess — and a non-zero exit is reported rather
  than read as an empty reply.
- **The prompt never enters the command line.** It is written to a file and read back with
  `"$(cat file)"`, so `; | & $( ) < >` and quotes are data. That is the same class of failure
  `bin/handoff.mjs` documents five times over. The one casualty is trailing newlines, which
  command substitution strips; `bin/peer/term.test.mjs` pins that rather than hiding it.
- **Continuity is opencode's own.** The session id is read out of the `--format json` event
  stream and passed back as `--session` next turn, so each turn is a fresh process and the
  conversation still remembers itself. `--new` starts over; `reset` forgets.

`--log` threads both halves of the exchange into `log.jsonl`. Without it the committed thread is
left alone, because a scratch question should not ship to the other machine. Scraping the real
TUI was the alternative and was rejected: it needs idle-detection, ANSI stripping and
bracketed-paste handling, and it breaks on every TUI redesign.
